  ...
  <div class="add-page fix-top">
    <div class="get-cd-info-box">
      ...
      <div class="title-info-box">
        ...
        <textarea class="cd-name-input" @focus="stopPlayBar(true)" @blur="stopPlayBar(false)" 
        rows="5" placeholder="请输入简介"></textarea>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      save () {
        if (!this.upimg || !this.title) {
          this.$Message.warning('还没填完 草peko');
        } else {
          this.$http.post(`${getAPI()}/addCd`, {
            'title': this.title,
            'imgUrl': this.upimg
          }).then(res => {
